...
public interface NewsRepository extends JpaRepository<News, Integer> {
    ...
    @Query("Select n from News n where n.title like %:title% and n.isDeleted = :isDeleted and n.workplace.id = :workplaceId ORDER BY n.dateModified DESC")
    ...
    @Query(value = "FROM News n WHERE n.dateCreated >= ?1 AND n.dateCreated <= ?2 AND n.workplace.id = ?3 ORDER BY n.dateModified DESC")
    List<News> findNewstByDateCreated(Date startDate, Date endDate, Integer workplaceId);

    @Query("SELECT n FROM News n WHERE n.workplace.id = :id AND n.isDeleted = false ORDER BY n.dateModified DESC")
