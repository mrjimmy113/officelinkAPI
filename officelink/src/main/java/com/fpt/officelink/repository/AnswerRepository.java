...
public interface AnswerRepository extends JpaRepository<Answer, Integer> {
	...
	@Query("SELECT COUNT(a) FROM Answer a WHERE a.assignmentHistory.account.id = :id AND a.surveyQuestion.survey.id = :surveyId")
...
//	List<AnswerReportDTO> testReport();
	...
	@Query("SELECT a FROM Answer a WHERE a.assignmentHistory.account.id = :id")
	Set<Answer> findAllByAccountId(@Param("id") int id);

	@Query("SELECT a FROM Answer a WHERE a.surveyQuestion.survey.id = :surveyId AND a.assignmentHistory.account.id = :accountId")
