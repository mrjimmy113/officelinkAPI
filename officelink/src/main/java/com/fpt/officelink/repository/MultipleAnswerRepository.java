...
//
//	@Query("SELECT NEW AnswerReport(w.answerOption, 1) FROM MultipleAnswer w JOIN w.answer.account.teams t WHERE w.answer.surveyQuestion.id = :id AND t.department.id = :departmentId GROUP BY w.id")
...
//
//	@Query("SELECT NEW AnswerReport(w.answerOption, 1) FROM MultipleAnswer w JOIN w.answer.account.teams t WHERE w.answer.surveyQuestion.id = :id AND w.answer.account.location.id = :locationId AND t.department.id = :departmentId GROUP BY w.id")
...
//
//	@Query("SELECT NEW AnswerReport(w.answerOption, 1) FROM MultipleAnswer w JOIN w.answer.account.teams t WHERE w.answer.surveyQuestion.id = :id AND (w.answer.account.location.id = :locationId OR t.department.id = :departmentId) GROUP BY w.id")
